...
class RollingQueue {
   public:
    ...
    int32_t LIndex;
    RollingQueue()
        : cap(RollingQueueInitialSize), size(0), head(0), tail(0), data(new Zombie*[RollingQueueInitialSize]()), RIndex(0), LIndex(0) {}
    ~RollingQueue() {
        delete[] data;
    }
    void rewind();
...
inline Zombie* RollingQueue::getNext() {
    if (RIndex == tail)
        return nullptr;
